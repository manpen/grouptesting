...
public:
    ...
    void generate_design_gamma(pat_id num_patients, test_id num_tests, double patients_per_test, std::mt19937_64& gen) {
        // Setup model
        num_patients_ = num_patients;
        num_tests_ = num_tests;
        patients_per_tests_ = patients_per_test;
        num_edges_ = std::round(num_tests * patients_per_test);
        ...
        // draw ~tests_per_patient_ balls per patient and remove duplicates
        {
            ...
            for (pat_id u = 0; u < num_patients_; ++u) {
                const auto take = edges_per_patient + (u < patients_with_extra);
    ...
    void cough(double theta, std::mt19937_64& gen) {
        ...
        if (5 * num_infected > num_patients()) {
            std::vector<pat_id> patients(num_patients_);
            std::iota(patients.begin(), patients.end(), 0u);
            tlx::random_bipartition_shuffle(patients.begin(), patients.end(), num_infected, gen);
            patients.resize(num_infected);
            for(auto patient_idx : patients) {
                auto res = infected_patients_.insert(patient_idx).second;
                assert(res);
